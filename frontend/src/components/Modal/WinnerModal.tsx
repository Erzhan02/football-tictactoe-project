...
const WinnerModal: React.FC<WinnerModalProps> = ({
  isOpen, status, winner, winnerName, scoreX, scoreO, playerXName, playerOName, onReset, onClose,
}) => {
      ...
      <div className="text-center space-y-6">
        ...
        >
          <div className="text-center">
            ...
            <p className="text-white/50 text-xs font-semibold mt-1">{playerXName || 'ИГРОК X'}</p>
          </div>
          <div className="text-white/20 text-2xl font-thin">:</div>
          <div className="text-center">
            <p className="text-rose-400 font-black text-3xl">{scoreO}</p>
            <p className="text-white/50 text-xs font-semibold mt-1">{playerOName || 'ИГРОК O'}</p>
          </div>
        </div>
